...
def theta_star(i: int, X: np.ndarray):
    ...
    """
    return np.tanh(X.sum(axis=1) / np.sqrt(50)) / 0.628 * np.sin(i * np.pi / 8.0)
...
x = np.random.uniform(-np.sqrt(3), np.sqrt(3), size=(100000, 50))
within_bounds = x.sum(axis=1) / np.sqrt(50) > domain_lower_bound
...
def generate_X_Y(A, L):
    ...
    for i in range(n):
        for j in range(i):
            if A[i, j] == 1:
                X_ijl[i, j, :, :] = np.random.uniform(-np.sqrt(3), np.sqrt(3), size=(L, 50))
                X_ijl[j, i, :, :] = X_ijl[i, j, :, :]
...
def initialize_networks(k, input_size = 50, hidden_size = 128, output_size = 1):
    """
...
def estimate_Q(networks, results, splitted_dense_data):
    ...
    for s in range(S):
        ...
        for data in splitted_dense_data[s]:
            ...
            theta_i0 = networks[s][i_0 - 1](torch.tensor(data[2], dtype=torch.float, device=device))
            theta_j0 = networks[s][j_0 - 1](torch.tensor(data[2], dtype=torch.float, device=device))
...
# estimated_Sigma_list = estimate_Sigma(networks, A, splitted_dense_data)
...
def estimate_V(networks, splitted_dense_data, estimated_Q, estimated_Sigma_list):
    ...
    for s in range(S):
        ...
        for data in splitted_dense_data[s]:
            ...
            sub_theta_i0_j0 = networks[s][i_0 - 1](torch.tensor(data[2], dtype=torch.float, device=device)) - networks[s][j_0 - 1](torch.tensor(data[2], dtype=torch.float, device=device))
